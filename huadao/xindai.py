...
def analysis_data():
    ...
    filename = '北京钱到到金服科技有限公司信贷详情2018.4.20'
    data_1 = pd.read_excel(path+filename+'.xlsx', sheetname='信贷详情', header=0)
    data_2 = pd.read_excel(path+filename+'.xlsx', sheetname='Table1', header=0)
    # print(data_1)
    ...
    writer = pd.ExcelWriter(path + '华道信贷结果划分.xlsx')
    sheet_list = []
    for k, v_data in data_dict.items():
        ...
        for mobile, group in v_data.groupby('手机号'):
            ...
            data_14 = group[(group['delta_days'] <= 14) & (group['delta_days'] >= 0)]
            data_30 = group[(group['delta_days'] <= 30) & (group['delta_days'] >= 0)]
            data_60 = group[(group['delta_days'] <= 60) & (group['delta_days'] >= 0)]
            ...
            user_dict['近7天'] = data_7
            user_dict['近14天'] = data_14
            user_dict['近30天'] = data_30
            user_dict['近60天'] = data_60
            user_dict['近90天'] = data_90
            user_dict['近180天'] = data_180
            for k_2, v_2 in user_dict.items():
                result_dict[k_2 + '_' + k + '次数'] = len(v_2[k + '平台'].notnull())
            result_list.append(result_dict)
        result = DataFrame(result_list, columns=['手机号','所有日期_'+k+'次数', '近7天_'+k+'次数', '近14天_'+k+'次数', '近30天_'+k+'次数',
                                                 '近60天_'+k+'次数', '近90天_'+k+'次数', '近180天_'+k+'次数'])
