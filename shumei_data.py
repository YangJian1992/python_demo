...
SELECT 
    ...
    new_data.auth_time,
    ulo.max_overdue_days
FROM
    (SELECT 
        id, user_id, auth_status, create_time, auth_time, device
    FROM
        credit_apply_orders
    GROUP BY user_id
    HAVING auth_time = MAX(auth_time)) AS new_data
        INNER JOIN
    loan_system_auth_record AS lsar ON new_data.id = lsar.loan_id
        LEFT JOIN
    (SELECT 
        user_id, MAX(overdue_days) AS max_overdue_days
    FROM
        user_loan_orders
...
def analysis_data():
    data = pd.read_table(path + file_name, encoding='utf-8')
    # st = data['risks'].values[0][1:-1]
    # print(st)
    st = '''
    {
"110000":"北京市",
...
"820109":"澳门新城"
    ...
    '''
    #把json数据转换成Series格式
    se = pd.read_json(st, typ='series')
    print(se)

#两函数都用到了path和file_name
path = 'D:\\work\\database\\shumei\\'
file_name = 'shumei_data.csv'
st = '''
    {
